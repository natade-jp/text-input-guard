...
単なる入力制限を行うだけでなく、入力中の体験と確定後のデータ整合性の分離、及び入力全体のフローを分離し拡張性を高める設計としていきます。
...
この問題を回避するためには、`pattern` を使用した入力制限や英語圏のライブラリを使用するといった方法があります。
しかし、簡単に回避できるものでもなく以下のような問題が不具合として発生する場合があります。
...
他の案として日本圏のライブラリとしては、`InputManJS` というものがありますが、より多機能なライブラリとなっており、inputの1つのために導入するのはコストが高く、気軽に試せるものではありません。

これらの問題を解決するために、開発を行おうと判断しました。

### 1.3 設計原則

TextInputGuard は、使用感の向上と拡張性のため次の原則を採用しています。
...
3. フェーズを明確に分解する
    ...
    これらを段階的に適用することで、問題を起こりにくくし、挙動を予測可能にします。
...
後述の `validate` で実施する「エラー表示」と「入力キャンセル」は別概念です。
...
通常は、`input`は1つなのですが、カンマ区切りなど表示（以下display）の`input`と、formで送信する際などの実際（以下raw）の`input`とを、分ける機能を持っています。

この機能は、`format` のルールを適用した際に動作します。

具体的には次の動作を行います。

1. HTML上に設置した`input`を、raw用としてhidden化
2. 全く同一のdisplay用の`input`を表示させる
3. 元々、設置していた`input`についていた要素を全てdisplay用の`input`へ付け替える動作（以下swap）を実施

### 2.4 キャレット保持

`normalizeChar` や `normalizeStructure` による値変更時に、可能な限りキャレット位置を維持します。
...
引数は、IMEで入力後の確定した文字が入ります。

IMEで入力確定した文字の一部分のみを消すという操作が可能のため、
一部のルールではここで文字を削除しています。
...
ルール違反を判定し、以下のいずれかを実施することができます。

- `ctx.pushError()` … エラー表示のみ
- `ctx.requestRevert()` … 入力キャンセル（IMEの確定キャンセル）
