...
test("bytes - normalizeChar: overflowInput が block 以外なら何もしない", () => {
	const rule = bytes({ max: 4, overflowInput: "error", unit: "utf-8" });
...
test("bytes - normalizeChar: max 未指定なら制限なし", () => {
	const rule = bytes({ overflowInput: "block", unit: "utf-8" });
...
test("bytes - normalizeChar: すでに beforeText が max 以上なら追加は全カット", () => {
	...
	const rule = bytes({ max: 4, overflowInput: "block", unit: "utf-8" });
...
test("bytes - normalizeChar: utf-16 はコードユニット数×2（サロゲートペアは 4bytes）", () => {
	...
	// max=2 なら追加不可（4 > 2）
	{
		const rule = bytes({ max: 2, overflowInput: "block", unit: "utf-16" });
	...
	// max=4 なら追加可能
	{
		const rule = bytes({ max: 4, overflowInput: "block", unit: "utf-16" });
		const ctx = makeCtx({ beforeText: "", insertedText: smile });
		const out = rule.normalizeChar(smile, ctx);
...
test("bytes - normalizeChar: utf-32 はコードポイント数×4（結合文字は別コードポイントで 8bytes）", () => {
	...
	const rule = bytes({ max: 4, overflowInput: "block", unit: "utf-32" });
...
test("bytes - normalizeChar: sjis/cp932 は Shift_JIS エンコード長で判定（ASCII 1byte, 全角 2byte 想定）", () => {
	...
	const rule = bytes({ max: 3, overflowInput: "block", unit: "sjis" });
...
test("bytes - normalizeChar: sjis/cp932 で超過する場合は入るところまで切る", () => {
	...
	const rule = bytes({ max: 3, overflowInput: "block", unit: "cp932" });
...
test("bytes - validate: overflowInput=error かつ max 超過ならエラー（utf-8）", () => {
...
test("bytes - validate: overflowInput が error 以外なら何もしない", () => {
	const rule = bytes({ max: 4, overflowInput: "block", unit: "utf-8" });
...
test("bytes - validate: max 未指定なら制限なし", () => {
	const rule = bytes({ overflowInput: "error", unit: "utf-8" });
...
test("bytes - fromDataset: オプションが正しく反映される（utf-16）", () => {
	const dataset = {
		tigRulesBytes: "1",
		tigRulesBytesMax: "3",
		tigRulesBytesOverflowInput: "error",
