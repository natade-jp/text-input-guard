...
test("width - normalizeChar: mode が block 以外なら何もしない", () => {
	const rule = width({ max: 4, mode: "error" });
...
test("width - normalizeChar: ASCII は 1幅、全角は 2幅として超過分がカットされる", () => {
	...
	const rule = width({ max: 4, mode: "block" });
...
test("width - normalizeChar: 半角カタカナは 1幅として数える", () => {
	...
	const rule = width({ max: 3, mode: "block" });
...
test("width - normalizeChar: 結合文字など 0幅要素は幅に影響しない（e + combining acute）", () => {
	...
	const rule = width({ max: 1, mode: "block" });
...
test("width - validate: mode=error かつ max 超過ならエラー", () => {
	// "ab"(2) + "あ"(2) = 4 > 3
	const rule = width({ max: 3, mode: "error" });
